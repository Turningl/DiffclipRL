...
from diffclipRL.mattergen.diffusion.corruption.sde_lib import SDE, Corruption
...
def apply(fns: Dict[str, Callable[..., R]], broadcast, **kwargs) -> Dict[str, R]:
    ...
    for field_name, fn in fns.items():
        fn(**{k: v[field_name] for k, v in kwargs.items() if field_name in v}, **(broadcast or dict()), )
