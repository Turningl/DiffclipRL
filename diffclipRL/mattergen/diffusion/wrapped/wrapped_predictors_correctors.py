...
import diffclipRL.mattergen.diffusion.sampling.predictors_correctors as pc
from diffclipRL.mattergen.diffusion.corruption import sde_lib
...
from diffclipRL.mattergen.diffusion.exceptions import IncompatibleSampler
from diffclipRL.mattergen.diffusion.sampling import predictors
from diffclipRL.mattergen.diffusion.wrapped.wrapped_sde import WrappedSDEMixin
...
SampleAndMeanAndStd = Tuple[torch.Tensor, torch.Tensor, torch.Tensor]
...
class WrappedPredictorMixin:
    ...
    def update_given_score(
        ...
        *,
        ...
        sample, mean, std = _super.update_given_score(
            x=x, t=t, dt=dt, batch_idx=batch_idx, score=score, batch=batch
        )
        return self.corruption.wrap(sample), self.corruption.wrap(mean), self.corruption.wrap(std)
